...
import { signOut } from '~/redux/_modules/auth/actions';
...
const UserDropdown = (props) => {
  ...
  return (
    <DropdownMenu right>
      ...
      <DropdownItem tag="a" href="#" onClick={(e) => dispatch(signOut())}>
      ...
      <UncontrolledDropdown tag="li" className="dropdown-user nav-item">
        <DropdownToggle tag="a" className="nav-link dropdown-user-link">
          <div className="user-nav d-sm-flex d-none">
            <span className="user-name text-bold-600">{profile.name}</span>
            <span className="user-status">Disponível</span>
          </div>
          <span data-tour="user">
            <img
              src={`https://api.adorable.io/avatars/285/${profile.name}.png`}
              className="round"
              height="40"
              width="40"
              alt="avatar"
